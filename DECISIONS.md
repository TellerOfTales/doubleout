...
28. **The Thin** library (12 cards): S20×2, S19, S18, S16, S12, S7, S3, T20, T19, D16, D20.
29. **The Sharp** replaces the two S1 cards with T18 and T17.
...
37. **Commit gesture**: flick (drag up and release with upward velocity, or lift the card past a
    ...
    H toggles the hint, Esc pauses.
38. **Checkout hint threshold** is `max(170, 3 × best single-card value with current chalk)` so
    inflated builds still see routes; the route search uses the real pipeline without wired
    deflection, respects the current throw index, and never relies on forgiveness.
